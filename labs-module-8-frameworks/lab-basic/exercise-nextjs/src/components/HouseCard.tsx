...
export function HouseCard({ house }: HouseCardProps) {
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl border border-neutral-200 transition-shadow hover:shadow-lg">
      <Link href={`/houses/${house.id}`} className="contents">
      ...
      <div className="absolute right-3 top-3 z-10">
        <ReserveButton houseId={house.id} houseName={house.name} />
      </div>
    </div>
  );
}
